...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	...
	for (int i = 0; i < particles.size() ; ++i) {
		...
		x_pred = x + velocity/yaw_rate*(sin(theta+yaw_rate*delta_t)-sin(theta)) + noise_x(gen);
		y_pred = y + velocity/yaw_rate*(cos(theta)-cos(theta+yaw_rate*delta_t)) + noise_y(gen);
		theta_pred = theta + yaw_rate*delta_t + noise_theta(gen);
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	double factor1 =  1.0 / ( s_y*s_x*2.0*M_PI) ;
	double weight_sum = 0;
	...
	for (int i = 0; i < particles.size() ; ++i) {
		...
		for (int j = 0; j < observations.size() ; ++j){
			...
			double obs_x = xt*cos(theta) - yt*sin(theta) + x;
			double obs_y = xt*sin(theta) + yt*cos(theta) + y;
			double dist_min = 100000.0;
			double pred_x_min;
			double pred_y_min;
			...
			double P_xy = factor1 * exp( -(0.5*pow( (obs_x-pred_x_min)/s_x, 2.0 )+0.5*pow( (obs_y-pred_y_min)/s_y, 2.0 ) ));
